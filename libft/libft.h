...
/**
 * search the character c  is search in the string s and if it´s found return a pointer to the first occurence is returned
...
/**
 * check if the charachter is an uppercase and convert in lower adding 32 in ascii
...
/** 
 *check if the charachter is an lowercase and convert in uppercase resting 32 in ascii
...
/**
 * set the first 'num' bytes of the memory are pointed to by 'value' to the specified value
 * 
 * Param ptr pointer to the memory area to be filled
 * Param value Value to be set it star as an int but when it filled the memory using the unsigned char conversion
...
/**
 * this function take the value of the str and converts it ti an int it skip spaces and othrwise if it is (- || +) only keep the -
 * and use if it we need to change the sign and next convert the char numbers into int number
 * 
...
/**
 *this function copis the string pointed to by 'src' to the buffer pointed to by 'dst'
 *the copied operation will stop when either the null-teminating character of 'src' is reached or when
...
/**
 * This function searches for the last occurence of the character 'c' in the string 'str'
 * If the charaacter is found a pointer to the last occurence is returned
 * if the character in not found and 'c' is null a pointer to the null terminator of the 'str' is returned
 * if the character is not found it return '\0'
 * 
 * Param str The string to be searched
 * Param c The character to be found 
 * 
 * Return a pointer to the last occurence of the character in the string || null if not found
 */
...
/**
 * this function compares the string pointed by 's1' and second string pointed by 's2' with a maximum of 'c' character
 * 
 * Param str First string
 * Param str2 Second string
 * Param c Maximum of cahracter
 * 
 * return An int greater, equal or less than 0 depending on the diferences betew str and str2
 */
...
/**
 * This function compare the first 'n' bytes of the first string 'dst' and the second one 'str'
 * 
 * Param dst First string
 * Param str Second string
 * Param c Number of bytes to compare
 * 
 * return An int greater, equal or less than 0 depending on the diferences betew str and dst
 */
 ...
 *  
 * return Pointer to the first occurrence of the byte if found or null if not found
 */
...
/**
 * this function take two null terminated string s1 and s2 and cancatenated them into a newly allocated string
 * the new string is dynamycally allocated and must be free by the caller
 * 
 * Param s1 the first string
 * Param s2 the second string 
 * 
 * Return a pointer to the newly allocated string of the result of the concatenated s1 and s2
 */
